...
class Fly(object):
    ...
    def update(self):
        ...
        if (self.wudi/2)%2==0 or self.wudi<70 :
            ...
            else :screen.blit(guang, (self.px, self.py))
            if self.xiao==1:
...
class Enemy1(object):
    ...
    def up(self):
        if moshi==0 and self.vy!=0:
            ...
            if x!=0 :
                k=1
                if (x>0 and x<30) or (x<-30):k=-1
                self.vx+=abs(2)/3 *k
            if abs(self.vx)>2:self.vx=2*self.vx/abs(self.vx)
        else :
...
if __name__ == '__main__':
    ...
    if moshi==0:
        ...
        while True:
            ...
            if fly.score==0:
                ting *= -1;
                screen.blit(font.render('Press p to be continue', -1, (255, 0, 0)), (150, 300))
                pygame.display.update()  # 更新显示
                fly.score=1
                continue
            time2 += 1
